                  ...
                  <div className="w-[340px] bg-white/95 backdrop-blur-2xl rounded-2xl border border-black/[0.06] shadow-[0_20px_60px_-12px_rgb(0,0,0,0.12),0_0_0_1px_rgb(0,0,0,0.03)] p-2.5">
                    {nav.items.map((item, idx) => (
                      <Link
                        ...
                        className="flex items-start gap-4 p-3.5 rounded-xl hover:bg-black/[0.03] transition-all duration-200 group/item animate-fade-in-up"
                        style={{ animationDelay: activeDropdown === nav.label ? `${0.05 + idx * 0.05}s` : '0s' }}
                      >
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-accent/10 to-accent/5 flex items-center justify-center text-accent/80 group-hover/item:from-accent group-hover/item:to-accent/80 group-hover/item:text-white transition-all duration-300 shrink-0 group-hover/item:scale-110">
